...
(function () {
  ...
  function showLoginGate() {
    ...
    o.id = 'loginGate';
    o.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:center;min-height:100vh;min-height:100dvh;background:' +
      bg +
      ";padding:24px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif\">" +
      '<div style="width:100%;max-width:360px;text-align:center">' +
      '<div style="font-size:64px;margin-bottom:16px">📁</div>' +
      '<h1 style="font-size:24px;font-weight:700;color:' +
      text +
      ';margin:0 0 8px">Doc77</h1>' +
      '<p style="font-size:14px;color:' +
      subtext +
      ';margin:0 0 28px">请输入密码解锁</p>' +
      '<input id="loginPass" type="password" placeholder="密码" autocomplete="current-password" style="width:100%;box-sizing:border-box;padding:14px 16px;border:1px solid ' +
      inputBd +
      ';border-radius:12px;font-size:16px;background:' +
      inputBg +
      ';color:' +
      text +
      ';outline:none;margin-bottom:16px;-webkit-appearance:none" onkeydown="if(event.key===\'Enter\')unlock()">' +
      ...
      muted +
      ';margin-top:40px">Doc77 — 本地文档管理</p>' +
      '</div>' +
      '</div>';
    ...
    window.unlock = async function () {
      ...
      try {
        ...
        if (d.ok) {
          ...
          var card =
            document.querySelector('.login-gate-card') ||
            document.querySelector('[style*="max-width:360px"]');
          if (card) {
...
window.toast = function (msg, type) {
  ...
  el.style.cssText =
    'padding:10px 16px;border-radius:8px;font-size:13px;color:#fff;box-shadow:' +
    shadow +
    ';max-width:280px;font-family:-apple-system,BlinkMacSystemFont,sans-serif;animation:fadeIn .3s ease-out';
  el.textContent = msg;
