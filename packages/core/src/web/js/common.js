...
window.__doc77_i18n_ready = fetch(
  '/api/i18n?' +
    (function () {
      var o = localStorage.getItem('doc77_lang');
      return o
        ? 'lang=' + encodeURIComponent(o)
...
window.showProgressOverlay = function (title, steps) {
  ...
  var stepsHtml = steps
    .map(function (s, i) {
      var icon = s.icon || '○';
      return (
        '<div class="progress-step" data-step="' +
        i +
  ...
  return {
    advance: function (stepIndex) {
      ...
      if (fill)
        fill.style.width = Math.min(100, Math.round((stepIndex / (all.length - 1)) * 100)) + '%';
    },
...
(function () {
  ...
  window.confirmDialog = function (msg) {
    return new Promise(function (resolve) {
      ...
      o.innerHTML =
        '<div class="confirm-box"><p style="font-size:13px;margin-bottom:16px;color:var(--text-primary)">' +
        msg +
        '</p>' +
      ...
      document.body.appendChild(o);
      o.querySelector('.ok-btn').onclick = function () {
        o.remove();
        resolve(true);
      };
      o.querySelector('.cancel-btn').onclick = function () {
        o.remove();
        resolve(false);
      };
      ...
      o.innerHTML =
        ...
        (msg
          ? '<p style="font-size:13px;margin-bottom:12px;color:var(--text-secondary)">' +
            ...
            '</p>'
          : '') +
        '<input class="input prompt-input" type="' +
        type +
        '" placeholder="' +
        escAttr(placeholder) +
        '" style="width:100%;padding:8px 12px;margin-bottom:16px">' +
        '<div style="display:flex;gap:8px;justify-content:flex-end">' +
        '<button class="btn cancel-btn" style="font-size:12px">' +
        esc(cancelText) +
        '</button>' +
        '<button class="btn btn-primary ok-btn" style="font-size:12px">' +
        esc(okText) +
        '</button></div></div>';
      document.body.appendChild(o);
...
async function installElectronModule(mod) {
  ...
  try {
    var r = await fetch('/api/electron/install', {
      method: 'POST',
  ...
  if (btn) {
    btn.disabled = false;
    btn.textContent = '📦 一键安装 AI 模块';
  }
}

async function checkTranslateStatus() {
  try {
    var r = await fetch('/api/translate/status');
    var d = await r.json();
    var el = document.getElementById('translateModelStatus');
    if (!el) return;
    if (!d.engineAvailable) {
      el.innerHTML = '<span style="color:var(--danger)">❌ 引擎未安装</span>';
    } else {
      var parts = [];
      parts.push(
        d.models && d.models['en-zh'] ? '✅ English → 中文' : '⬜ English → 中文（未下载）',
      );
      parts.push(
        d.models && d.models['zh-en'] ? '✅ 中文 → English' : '⬜ 中文 → English（未下载）',
      );
      el.innerHTML = parts.join('<br>');
    }
  } catch (e) {
    var el2 = document.getElementById('translateModelStatus');
    if (el2) el2.innerHTML = '<span style="color:var(--text-muted)">无法检查状态</span>';
  }
}
...
async function downloadTranslateModels() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.textContent = '下载中...（首次约需 1-2 分钟）';
  }
  try {
    var mirrorCB = document.querySelector('[data-key="translate.mirror"]');
    if (mirrorCB && mirrorCB.checked) {
      await fetch('/api/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: 'translate.mirror', value: 'true' }),
      });
    }
    ...
    if (r.ok) {
      toast('✅ en-zh 模型下载完成', 'success');
    } else {
      var d = await r.json();
      toast('下载失败，请终端运行: doc77 vendor-install --translate en-zh --mirror', 'error');
      if (btn) {
        btn.disabled = false;
        btn.textContent = '📥 下载翻译模型 (~160MB)';
      }
...
function switchSettingsTab(tab) {
  ...
  if (tab === 'system') {
    ...
    if (!window.__doc77_caps_ai) {
      ...
      c.innerHTML =
        '<div style="text-align:center;padding:32px 0;font-size:13px;color:var(--text-muted)">AI 模块未安装<br>' +
        electronInstall +
        '</div>';
      return;
    }
    c.innerHTML =
      ...
      settingToggle('启用 AI', 'ai.enabled') +
      settingToggle('自动模式', 'ai.auto_mode') +
      settingRow('风险等级', 'ai.risk_level', 'select', 'medium', 'low,medium,high') +
      settingToggle('确认删除', 'ai.confirm_delete') +
      settingRow('批量大小', 'ai.batch_size', 'number', '5') +
    ...
    if (!window.__doc77_caps_translate) {
      var electronInstall = window.doc77
        ? '<button onclick="installElectronModule(\'translate\')" id="btnInstallTranslate" class="btn btn-primary" style="display:inline-flex;margin-top:12px;font-size:13px">📦 一键安装翻译模块</button>'
        : '<code style="font-size:11px;background:var(--bg-code);padding:4px 8px;border-radius:4px;display:inline-block;margin-top:8px;color:var(--text-primary)">doc77 i translate</code>';
      c.innerHTML =
        '<div style="text-align:center;padding:32px 0;font-size:13px;color:var(--text-muted)">翻译模块未安装<br>' +
        electronInstall +
        '</div>';
      return;
    }
    c.innerHTML =
      ...
      settingToggle('启用离线翻译', 'translate.enabled') +
      settingToggle('使用国内镜像 (hf-mirror.com)', 'translate.mirror') +
      settingRow('默认源语言', 'translate.default_source', 'text', 'auto') +
      settingRow('默认目标语言', 'translate.default_target', 'text', 'zh') +
...
function settingRow(label, key, type, placeholder, opts) {
  ...
  if (type === 'password')
    h =
      '<button style="position:absolute;right:4px;top:50%;transform:translateY(-50%);font-size:11px;color:var(--text-muted);border:none;background:none;cursor:pointer;line-height:1" onclick="togglePasswordView(this)">👁</button>';
  return (
    '<div class="settings-row"><span class="settings-label">' +
    label +
    '</span>' +
    '<div style="position:relative"><input data-key="' +
    key +
    '" type="' +
    tt +
    '" placeholder="' +
    (placeholder || '') +
    '" class="input" style="width:176px">' +
    h +
    '</div></div>'
  );
}
function settingToggle(label, key) {
  return (
    ...
    '<button data-key="' +
    key +
    '" data-value="false" onclick="toggleSwitch(this)" class="toggle-track">' +
...
function togglePasswordView(btn) {
...
function langSelect(id) {
  ...
  return (
    '<select id="' +
    id +
    '" onchange="onLangChange(this)" class="settings-select">' +
    opts +
    '</select>'
  );
}
...
// AI Providers
var AI_PROVIDERS = {
  deepseek: { url: 'https://api.deepseek.com', models: ['deepseek-v4-pro', 'deepseek-v4-flash'] },
  openai: { url: 'https://api.openai.com/v1', models: ['gpt-4o', 'gpt-4o-mini', 'gpt-5'] },
  qwen: {
    url: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
    models: ['qwen3-max', 'qwen-plus', 'qwen-turbo'],
  },
  kimi: { url: 'https://api.moonshot.cn/v1', models: ['kimi-k2.7-code'] },
  doubao: { url: 'https://ark.cn-beijing.volces.com/api/v3', models: ['doubao-seed-2-1-pro'] },
...
async function testConnection() {
  ...
  try {
    var res = await fetch('/api/ai/test', { method: 'POST' });
    var d = await res.json();
    if (d.ok) {
      r.textContent = '✅ 连接成功 (' + d.status + ')';
      r.style.cssText = 'font-size:11px;color:#059669';
    } else {
      r.textContent = '❌ ' + d.error;
      r.style.cssText = 'font-size:11px;color:var(--danger)';
    }
  } catch (ex) {
    r.textContent = '❌ 网络错误: ' + ex.message;
    r.style.cssText = 'font-size:11px;color:var(--danger)';
  }
}
  ...
  try {
    ...
    document.querySelectorAll('#settingsContent [data-key]').forEach(function (el) {
      ...
      if (el.tagName === 'SELECT') {
        ...
        el.dataset.value = v === 'true' ? 'true' : 'false';
        el.classList.toggle('on', v === 'true');
        if (el.querySelector('span')) el.querySelector('span').classList.toggle('on', v === 'true');
      } else {
...
async function saveSettings() {
  ...
  for (var i = 0; i < els.length; i++) {
    ...
    if (v === '' || (typeof v === 'string' && v.indexOf('•') !== -1)) {
      if (
        ['token', 'secret', 'password', 'apikey', 'api_key', 'authorization'].some(function (part) {
          return k.toLowerCase().indexOf(part) !== -1;
        })
      ) {
        continue;
      }
...
async function restartServer() {
  if (!(await confirmDialog('确定重启服务？正在进行的操作会中断，浏览器需要刷新页面重新连接。')))
    return;
  try {
    await saveSettings();
...
async function renderAccountSection() {
  ...
  if (d.hasPassword) {
    try {
      ...
      if (rd.hasRecovery) {
        rsHtml =
          '<div style="font-size:11px;color:var(--text-muted);margin-top:4px">剩余恢复码：' +
          rd.remaining +
          ' / ' +
          rd.total +
          '</div>' +
  ...
  if (d.hasPassword) {
    s.innerHTML =
      '<div style="font-size:13px;color:var(--text-primary);margin-bottom:4px">密码已设置</div>' +
      rsHtml +
    ...
    s.innerHTML =
      '<div style="font-size:13px;color:var(--text-muted);margin-bottom:8px">尚未设置密码</div>' +
...
async function forceResetPw() {
  if (
    !confirm(
      '⚠️ 此操作将清除所有安全设置（密码、恢复码）和加密配置（AI Token 等），且不可撤销。\n\n确定要继续吗？',
    )
  )
    return;
...
function updateStrength() {
  ...
  var l = ['非常弱', '弱', '一般', '强', '非常强'],
    c = ['#ef4444', '#f97316', '#eab308', '#84cc16', '#22c55e'];
  var el = document.getElementById('pwStrength');
  el.textContent = '强度: ' + l[Math.min(s, 4)];
  el.style.cssText = 'font-size:11px;color:' + c[Math.min(s, 4)];
}
...
async function changePw() {
  ...
  try {
    var r = await fetch('/api/auth/change-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ old_password: c, new_password: n }),
    });
    var d = await r.json();
    if (d.ok) {
      if (d.recovery_codes) {
...
async function regenerateRC() {
  ...
  try {
    var r = await fetch('/api/auth/recovery-codes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      if (d.isLegacy) {
          ...
          try {
            ...
            if (rd.ok) {
              ...
              prog.complete(function () {
                if (rd.recovery_codes) {
                  showRecoveryCodesModal(rd.recovery_codes);
                }
        ...
        window.unlock = async function () {
          ...
          try {
            ...
            if (d2.ok) {
              sessionStorage.setItem('doc77-auth', '1');
              if (d2.recovery_codes) {
                showRecoveryCodesModal(d2.recovery_codes);
              }
      ...
      async function showSecurityPrompt() {
        try {
          ...
          if ((sd['ai.token'] || sd['ai.enabled'] === 'true') && !d.hasPassword) {
            ...
            sb.innerHTML =
              '<span style="color:var(--accent)">⚠️ 已配置 AI 模型但未设置访问密码，建议设置密码保护数据安全</span>' +
...
async function showForgotPassword() {
  ...
  if (!h) return;
  h.innerHTML =
    '<div class="login-gate-card"><div class="login-gate-brand"><div class="login-gate-brand-row"><img src="/assets/favicon.svg" alt="Doc77"><span class="login-gate-brand-name">Doc77</span></div><div class="login-gate-brand-desc">找回密码 &middot; 输入恢复码</div></div>' +
...
async function verifyRC() {
  ...
  try {
    var r = await fetch('/api/auth/forgot-password/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recovery_code: rc }),
    });
    var d = await r.json();
    if (d.ok) {
      forgotState = 'reset';
...
function showResetPassword() {
  ...
  if (!h) return;
  h.innerHTML =
    '<div class="login-gate-card"><div class="login-gate-brand"><div class="login-gate-brand-row"><img src="/assets/favicon.svg" alt="Doc77"><span class="login-gate-brand-name">Doc77</span></div><div class="login-gate-brand-desc">设置新密码 &middot; 恢复码验证通过</div></div>' +
...
async function doReset() {
  ...
  try {
    ...
    var r = await fetch('/api/auth/forgot-password/reset', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ reset_token: token, new_password: p }),
    });
    var d = await r.json();
    if (d.ok) {
  ...
  fetch('/api/shares')
    ...
    .then(function (shares) {
      ...
      if (!shares || shares.length === 0) {
        list.innerHTML =
          '<div style="padding:16px 0;text-align:center;color:var(--text-muted)">暂无活跃分享</div>';
        return;
      }
      var html = '';
      for (var i = 0; i < shares.length; i++) {
        ...
        html +=
          ...
          '</div>' +
          '<div style="font-family:monospace;font-size:11px;color:var(--text-secondary);word-break:break-all;margin-bottom:6px">' +
          escHtml(shareUrl) +
          '</div>' +
          '<div style="display:flex;justify-content:space-between;align-items:center">' +
          '<span style="color:var(--text-muted);font-size:11px">有效期至 ' +
          expiresStr +
          '</span>' +
          '<button onclick="revokeShare(\'' +
          s.token +
          '\', this)" style="padding:3px 10px;font-size:11px;color:#ef4444;border:1px solid #ef4444;background:transparent;border-radius:4px;cursor:pointer">撤销</button>' +
          '</div></div>';
      }
      list.innerHTML = html;
    })
    .catch(function () {
      ...
      if (list)
        list.innerHTML =
          '<div style="padding:16px 0;text-align:center;color:#ef4444">加载失败</div>';
    });
}
