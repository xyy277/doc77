...
// Chromium 把页面冻结进 bfcache 时，EventSource 连接不关闭（Issue
// 358538891），成为占用浏览器每源 6 连接槽位的"僵尸连接"——反复进出
// 页面 5-6 次后槽位占满，同源所有请求（含导航）永久排队 → 卡死。
// pagehide（persisted）时主动关闭连接释放槽位；pageshow（persisted）
// 恢复时重建。注意：绝不能用 unload/beforeunload（会移除 bfcache
// 资格，页面每次重建成本高）；pagehide 不破坏资格。
// 熔断计数（taskSseFailures）是 initTaskEvents 内局部变量，重建自动归零。
window.addEventListener('pagehide', function (ev) {
  if (ev.persisted && taskEventSrc) {
    taskEventSrc.close();
    taskEventSrc = null;
  }
});
window.addEventListener('pageshow', function (ev) {
  if (ev.persisted) initTaskEvents();
});
...
function initTaskEvents() {
  ...
  try {
    ...
    taskEventSrc.addEventListener('task:executed', function (e) {
      ...
      loadTasks();
      appendTaskReceipt(t('web.preview.task.taskExecuted', { id: d.task_id || '?' }));
    });
    taskEventSrc.addEventListener('task:failed', function (e) {
      ...
      loadTasks();
      appendTaskReceipt(
        t('web.preview.task.taskFailed', { id: d.task_id || '?' }) +
      ...
      var affected = tabStore
        ...
        .filter(function (tab) {
          ...
          if (
            opType === 'delete' &&
            d.path &&
            (tab.path === d.path || tab.path.indexOf(d.path + '/') === 0)
          )
            return true;
          return false;
        })
...
function getExternalModifiedBanner() {
  ...
  b.className =
    'hidden flex items-center gap-2 px-3 py-1.5 text-xs bg-amber-50 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 border-b border-amber-200 dark:border-amber-800';
  b.innerHTML =
    '<span>⚠️ ' +
    t('web.preview.edit.externalModified') +
    '</span>' +
    '<button class="ml-auto px-2 py-0.5 rounded border border-amber-300 dark:border-amber-700 hover:bg-amber-100 dark:hover:bg-amber-800" onclick="reloadDocAfterExternalChange()">' +
    t('web.preview.externalModified.reload') +
    '</button>' +
...
function applyCapabilities() {
  ...
  if (!CAPABILITIES.mcp) {
    ...
    if (tabQueue) tabQueue.style.display = 'none';
    // 智能归类 relies on write tools (batch_operations) — hide when MCP absent.
    var btnClassify = document.getElementById('btnClassify');
...
(async function boot() {
  ...
  try {
    await (window.__doc77_i18n_ready || Promise.resolve());
  } catch (e) {}
  try {
    var r = await fetch('/api/projects');
    projects = await r.json();
    proj = projects.find(function (p) {
      return p.id === parseInt(pid);
    });
    if (!proj) {
      toast(t('web.preview.projectNotFound'), 'error');
      location.href = '/';
      return;
    }
    ...
    if (directPath) {
      setTimeout(function () {
        navigateToFile(directPath);
      }, 400);
    }
    fetch('/api/projects/' + pid + '/touch', { method: 'POST' }).catch(function () {});
  } catch (e) {
    document.getElementById('projName').textContent = t('web.preview.loadFailed');
  }
})();
...
function toggleProjMenu() {
  ...
  if (!menu.classList.contains('hidden')) {
    ...
    setTimeout(function () {
      var s = document.getElementById('projSearch');
      if (s) s.focus();
    }, 50);
  }
}
function renderProjMenu(filter) {
  ...
  var filtered = filter
    ? sorted.filter(function (p) {
        return (
          p.name.toLowerCase().indexOf(filter) >= 0 || p.path.toLowerCase().indexOf(filter) >= 0
        );
      })
    : sorted;
  var items = filtered
    .map(function (p) {
      return (
        '<button onclick="switchProject(' +
        p.id +
        ')" class="w-full text-left px-3 py-2 text-sm text-slate-200 hover:bg-slate-700 flex flex-col transition-colors"><span class="font-medium">' +
        (p.id === proj.id ? '✓ ' : '') +
        '💼 ' +
        esc(p.name) +
        '</span><span class="text-xs text-slate-500 truncate">' +
        esc(p.path) +
        '</span></button>'
      );
    })
  ...
  if (!items)
    items =
      ...
      t('web.preview.noMatchingProjects') +
      '</div>';
  document.getElementById('projMenu').innerHTML =
    '<div class="px-2 pt-2 pb-1 sticky top-0 bg-slate-800 z-10"><div class="relative"><span class="absolute left-2 top-1/2 -translate-y-1/2 text-slate-500 text-xs">🔍</span><input id="projSearch" placeholder="' +
    t('web.preview.searchProject') +
    '" oninput="renderProjMenu(this.value)" class="w-full bg-slate-700 border border-slate-600 rounded-md pl-7 pr-2 py-1.5 text-xs text-slate-200 outline-none focus:ring-1 focus:ring-blue-500 placeholder:text-slate-500"></div></div>' +
    '<div class="max-h-72 overflow-y-auto scrollbar-dark">' +
    items +
    '</div>' +
    '<div class="h-px bg-slate-700 my-1"></div><a href="/" class="block w-full text-left px-3 py-2 text-sm text-blue-400 hover:bg-slate-700 transition-colors">' +
    t('web.preview.registerNewProject') +
    '</a>';
}
...
function togglePanel(side) {
  ...
  if (side === 'left') {
    ...
    if (h) {
      ...
      capsule.title = t('web.preview.expandPanel');
    } else {
      capsule.style.right = (parseInt(panel.style.width) || 320) - 4 + 'px';
      capsule.title = t('web.preview.collapsePanel');
    }
    icon.textContent = h ? '◀' : '▶';
  }
}
...
(function () {
    ...
    h.addEventListener('mousedown', function (e) {
      ...
      if (i === 0) {
        ...
        if (lp && lp.classList.contains('collapsed')) {
          ...
          lp.querySelectorAll('.collapsible-content').forEach(function (el) {
  ...
  document.addEventListener('mousemove', function (e) {
    ...
    if (t === 'left') {
      ...
      leftPanelExpandedWidth = w;
    } else {
      var rp = document.getElementById('rightPanel');
      var maxW = Math.floor(window.innerWidth / 3);
      var w = Math.max(200, Math.min(maxW, window.innerWidth - e.clientX));
      rp.style.width = w + 'px';
      var cb = document.getElementById('capsuleBtn');
      if (cb && !rp.classList.contains('hidden')) cb.style.right = w - 4 + 'px';
    }
  });
...
function setActiveTab(t) {
  ...
  ['tabOutline', 'tabChat', 'tabQueue'].forEach(function (id) {
    ...
    var isChat = id === 'tabChat',
      isOutline = id === 'tabOutline',
      isQueue = id === 'tabQueue';
    var active =
      (isChat && t === 'chat') || (isOutline && t === 'outline') || (isQueue && t === 'queue');
    el.className =
      'flex-1 py-3 text-sm font-medium flex items-center justify-center gap-1.5 transition-colors ' +
      (active
        ? isOutline
          ? 'text-emerald-600 dark:text-emerald-400 border-b-2 border-emerald-600 dark:border-emerald-400 bg-white dark:bg-slate-900'
          : isChat
...
async function loadTree(dirPath) {
  ...
  tree.innerHTML =
    '<div class="text-center py-4"><div class="skeleton h-4 w-3/4 mx-auto mb-2"></div></div>';
  try {
    var r = await fetch('/api/tree/' + pid + '?path=' + encodeURIComponent(dirPath || ''));
    var d = await r.json();
    tree.innerHTML = '';
    var fld = d.entries.filter(function (e) {
      return e.type === 'directory';
    });
    var fls = d.entries.filter(function (e) {
      return e.type === 'file';
    });
    fld.concat(fls).forEach(function (e) {
      tree.appendChild(makeNode(e, dirPath || ''));
    });
    if (!fld.length && !fls.length) tree.innerHTML = '' + t('web.preview.emptyDir') + '';
  } catch (e) {
    tree.innerHTML = '' + t('web.preview.loadFailed') + '';
  }
}
...
function applyDiff(container, oldEntries, newEntries, basePath) {
  ...
  // 清理遗留占位（空目录提示等非行元素）
  container
    .querySelectorAll(':scope > :not([data-name]):not(.ml-4):not(.tree-collapse-btn)')
    .forEach(function (el) {
      el.remove();
    });

  diff.removed.forEach(function (name) {
    removeTreeRow(container, name);
  });
  ...
  if (!newEntries.length) {
    ...
    if (!hasRows) {
      if (container.classList.contains('ml-4')) {
        ...
        container.innerHTML =
          '<div class="text-slate-600 text-xs py-1 pl-2">' + t('web.preview.emptyDir') + '</div>';
      }
    }
    return;
  }
  ...
  for (var i = 0; i < newEntries.length; i++) {
    ...
    if (row) {
      ...
      var sizeChanged =
        ...
        String(row.dataset.size) !== String(entry.size || '');
    ...
    for (var j = i + 1; j < newEntries.length; j++) {
      if (rows[newEntries[j].name]) {
        ref = rows[newEntries[j].name];
        break;
      }
    }
...
function refreshSubtree(dirPath) {
  ...
  while (cur) {
    ...
    if (row) {
      var wrapper = row.nextElementSibling;
      if (wrapper && wrapper.classList.contains('ml-4')) {
        container = wrapper;
        break;
      }
...
function applyFilter() {
  ...
  document.querySelectorAll('#tree [data-name]').forEach(function (el) {
...
function makeNode(entry, parentPath) {
  ...
  row.className =
    'flex items-center gap-1.5 py-1.5 px-2 rounded-md cursor-pointer transition-colors text-sm hover:bg-slate-800 text-slate-300';
  row.innerHTML =
    '<span class="w-4 shrink-0 text-center text-slate-500 text-xs">' +
    (isDir ? '▸' : '') +
    '</span>' +
    '<span class="' +
    (isDir ? 'text-blue-400' : 'text-slate-400') +
    ' shrink-0">' +
    (isDir ? '📁' : iconFor(entry.name)) +
    '</span>' +
    '<span class="truncate flex-1 tree-name">' +
    entry.name +
    '</span>' +
    (entry.size
      ? '<span class="text-[10px] text-slate-500 shrink-0 tree-size">' +
        fmtSize(entry.size) +
        '</span>'
      : '');
    ...
    row.addEventListener('click', function (e) {
      ...
      if (wrapper.classList.contains('hidden')) {
        if (!loaded) {
          ...
          wrapper.innerHTML =
            '<div class="text-slate-500 text-xs py-1 pl-2">' + t('web.preview.loading') + '</div>';
          fetch('/api/tree/' + pid + '?path=' + encodeURIComponent(childPath))
            .then(function (r) {
              return r.json();
            })
            .then(function (d) {
              wrapper.innerHTML = '';
              var f = d.entries.filter(function (e) {
                return e.type === 'directory';
              });
              var l = d.entries.filter(function (e) {
                return e.type === 'file';
              });
              if (!f.length && !l.length)
                wrapper.innerHTML =
                  '<div class="text-slate-600 text-xs py-1 pl-2">' +
                  t('web.preview.emptyDir') +
                  '</div>';
              else
                f.concat(l).forEach(function (e) {
                  wrapper.appendChild(makeNode(e, childPath));
                });
              var cb = document.createElement('div');
              cb.className =
                'tree-collapse-btn text-[10px] text-slate-500 hover:text-slate-300 cursor-pointer py-1 pl-2 select-none';
            ...
            .catch(function () {
              wrapper.innerHTML =
                '<div class="text-red-400 text-xs py-1 pl-2">' +
                t('web.preview.loadFailed') +
                '</div>';
            });
        } else {
          wrapper.classList.remove('hidden');
  ...
  row.addEventListener('contextmenu', function (e) {
    e.preventDefault();
    showCtxMenu(e.clientX, e.clientY, childPath, isDir ? 'directory' : 'file');
  });
  return frag;
}
...
function iconFor(n) {
  ...
  return ['md', 'markdown'].indexOf(e) >= 0
    ? '📝'
    : ['mermaid', 'mmd'].indexOf(e) >= 0
      ? '📊'
      : e === 'pdf'
        ? '📕'
        : ['png', 'jpg', 'jpeg', 'gif', 'svg', 'webp', 'bmp'].indexOf(e) >= 0
          ? '🖼'
          : ['ts', 'js', 'py', 'rb', 'go', 'rs', 'java', 'c', 'cpp'].indexOf(e) >= 0
            ? '💻'
            : ['json', 'yaml', 'yml', 'toml'].indexOf(e) >= 0
              ? '⚙'
              : '📄';
}
...
function scrollToLine(lineNum) {
  ...
  marker.style.cssText =
    'position:absolute;left:0;right:0;height:2px;background:var(--accent,#2563eb);opacity:0.8;z-index:10;pointer-events:none;transition:opacity .5s';
...
function buildDocHTML(path, d) {
  ...
  if (d.type === 'unsupported') {
    var labels = {
      video: t('web.preview.type.video'),
      audio: t('web.preview.type.audio'),
      archive: t('web.preview.type.archive'),
      font: t('web.preview.type.font'),
      database: t('web.preview.type.database'),
      design: t('web.preview.type.design'),
      binary: t('web.preview.type.binary'),
      gis: t('web.preview.type.gis'),
      '3d': t('web.preview.type.model3d'),
      ebook: t('web.preview.type.ebook'),
      document: t('web.preview.type.document'),
      spreadsheet: t('web.preview.type.spreadsheet'),
      presentation: t('web.preview.type.presentation'),
      too_large: t('web.preview.type.tooLarge'),
    ...
    html =
      '<div class="max-w-md mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 text-center">' +
      ...
      (d.modified
        ? '<span class="text-slate-500">' +
          t('web.preview.modifiedTime') +
          '</span><span class="text-slate-700 dark:text-slate-300">' +
          new Date(d.modified).toLocaleString(window.__doc77_lang || 'zh-CN') +
          '</span>'
        : '') +
      '</div>' +
      '<p class="text-xs text-amber-600 dark:text-amber-400 mb-4">⚠️ ' +
      t('web.preview.unsupportedFormat') +
      '</p>' +
      (d.temp
        ? ''
        : '<button onclick="revealFile(\'reveal\')" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors">📂 ' +
          t('web.preview.showInFolder') +
          '</button>') +
      '</div>';
  } else if (d.type === 'markdown' || d.type === 'mermaid' || d.type === 'code') {
    html =
      '<div id="docCard" class="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-8 sm:p-12 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700"><div class="doc-content text-slate-700 dark:text-slate-300" id="docContent">' +
      d.content +
      '</div></div>';
    if (d.type === 'code') {
      var ext = path.split('.').pop().toLowerCase();
      if (ext === 'html' || ext === 'htm') {
        html =
          ...
          '<button id="htmlToggleBtn" onclick="toggleHtmlPreview()" style="position:fixed;top:8px;left:50%;transform:translateX(-50%);z-index:10;padding:6px 16px;border:1px solid #e2e8f0;border-radius:8px;background:rgba(255,255,255,0.9);color:#475569;font-size:13px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.1);backdrop-filter:blur(4px);white-space:nowrap">' +
          t('web.preview.htmlToggle.preview') +
          '</button>' +
          '<div id="htmlCodeView">' +
          html +
          '</div>' +
          ...
          t('web.preview.htmlToggle.fullscreen') +
          '">⛶</button>' +
          '<iframe data-src="' +
          d.rawUrl +
          '" style="flex:1;border:none;border-radius:8px;width:100%;min-height:60vh"></iframe></div></div>' +
          '</div>';
      }
    }
  } else if (d.type === 'image') {
    html =
      '<div class="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-4 sm:p-8 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-center" style="min-height:60vh"><img src="' +
      d.rawUrl +
      '" alt="' +
      esc(path) +
      '" class="max-w-full max-h-[80vh] object-contain rounded-md shadow-sm cursor-pointer hover:opacity-90 transition-opacity" loading="lazy" onclick="openImageLightbox(\'' +
      escAttr(d.rawUrl) +
      "', '" +
      escAttr(path) +
      '\')" /></div>';
    ...
    html =
      '<div class="pdf-wrapper" id="pdfContainer" style="position:relative;width:100%;min-height:calc(100vh - 160px);display:flex;flex-direction:column">' +
      '<button onclick="togglePdfFullscreen()" style="position:absolute;top:8px;right:8px;z-index:5;width:36px;height:36px;border-radius:50%;border:none;background:rgba(0,0,0,0.5);color:#fff;font-size:18px;display:flex;align-items:center;justify-content:center;cursor:pointer;backdrop-filter:blur(4px)" title="' +
      t('web.preview.htmlToggle.fullscreen') +
      '">⛶</button>' +
      '<iframe src="' +
      d.rawUrl +
      '" style="flex:1;border:none;border-radius:8px;width:100%;min-height:60vh"></iframe></div>';
  } else {
    html =
      '<div class="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-8 sm:p-12 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700"><pre class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-mono">' +
      esc(d.content) +
      '</pre></div>';
  }
...
function renderDocNode(path, d) {
  ...
  if (d.type === 'docx' || d.type === 'xlsx') {
    pane.innerHTML =
      '<div class="h-full flex items-center justify-center"><div class="text-sm text-slate-500">' +
      t('web.preview.loading') +
      '</div></div>';
...
function afterActivate(path, d) {
  ...
  if (isTemp) {
    ['aiBtn', 'editBtn', 'externalEditBtn', 'revealBtn'].forEach(function (id) {
      var el = document.getElementById(id);
  ...
  var isEditable =
    !isTemp &&
    !isMobileDevice &&
    editableExts.some(function (ext) {
  ...
  if (runBtn) {
    ...
    if (d && d.type === 'code' && (ext === 'js' || ext === 'py')) {
      ...
      runBtn.dataset.lang = ext;
      runBtn.dataset.code = d.content;
    } else {
...
function renderGraphPanelContent(card, path, backlinks) {
  ...
  var html =
    '<div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-sm">';

  // 反向链接区
  html +=
    '<div class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">' +
    esc(t('web.preview.graph.backlinks')) +
    (backlinks.length ? ' (' + backlinks.length + ')' : '') +
    '</div>';
  if (backlinks.length === 0) {
    html +=
      '<div class="text-slate-400 dark:text-slate-500 text-xs mb-3">' +
      esc(t('web.preview.graph.noBacklinks')) +
      '</div>';
  } else {
    html += '<ul class="space-y-1 mb-3">';
    backlinks.forEach(function (b) {
      html +=
        '<li><a href="javascript:void(0)" class="text-blue-500 hover:underline" data-graph-backlink="' +
        esc(b.from_path) +
        '">' +
        esc(b.title || b.from_path) +
        '</a></li>';
    });
    html += '</ul>';
  }
  ...
  html +=
    '<div class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">' +
    esc(t('web.preview.graph.related')) +
    '</div>';
  html +=
    '<div class="text-slate-400 dark:text-slate-500 text-xs" data-graph-related>' +
    esc(t('web.preview.graph.loadingRelated')) +
    '</div>';
...
function activateTab(path, opts) {
  ...
  if (activeTabPath && activeTabPath !== path)
    tabScroll[activeTabPath] = document.getElementById('contentArea').scrollTop;
  ...
  if (rightPanel && !rightPanel.classList.contains('hidden')) {
    ...
    if (ctxB) {
      ctxB.classList.remove('hidden');
      document.getElementById('ctxText').textContent = path;
    }
  }
  renderTabBar();
  syncTreeActive(path);
  ...
  fetchDoc(path, wasMarked ? { fresh: true } : undefined)
    .then(function (d) {
      ...
      if (translateActive) {
        exitTranslateMode();
        return;
      } // skip render when translate active
      var pane = renderDocNode(path, d);
      if (!isHeavyDoc(path, d)) {
        paneCache[path] = pane;
        var evicted = tabStore.noteRendered(path);
    ...
    .catch(function () {
      ...
      host.innerHTML =
        '<div class="h-full flex flex-col items-center justify-center text-slate-400 gap-2"><span class="text-4xl">⚠️</span><p class="text-sm">' +
        t('web.preview.fileLoadFailed') +
        '</p></div>';
      updateExternalModifiedBanner();
    });
}
...
function closeTab(path) {
  ...
  if (r.active) {
    activeTabPath = null;
    activateTab(r.active, { silent: true });
  } else showEmptyState();
}
...
function renderTabBar() {
  ...
  bar.innerHTML = tabs
    .map(function (tab) {
      ...
      var cls =
        'group flex items-center gap-1.5 pl-2.5 pr-1.5 py-1.5 my-1 rounded-md cursor-pointer text-xs whitespace-nowrap border transition-colors ' +
        (active
          ? 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 font-medium shadow-sm'
      ...
      return (
        ...
        escAttr(tab.path) +
        '\')" onmousedown="onTabMouseDown(event, \'' +
        escAttr(tab.path) +
        '\')">' +
        '<span class="shrink-0">' +
        iconFor(tab.title) +
        ...
        esc(tab.title) +
        '</span>' +
        '<button class="shrink-0 w-4 h-4 rounded flex items-center justify-center text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-red-500" onclick="event.stopPropagation();closeTab(\'' +
        escAttr(tab.path) +
        '\')" title="' +
        t('web.preview.close') +
        '">✕</button>' +
...
function onTabClick(e, path) {
  ...
  if (editMode && editDirty) {
    showEditConfirm(t('web.preview.edit.unsavedTitle'), t('web.preview.edit.switchSavePrompt'), [
      {
        text: t('web.preview.edit.saveAndSwitch'),
        cls: 'btn-primary',
        action: function () {
          doSave(function () {
            doExitEdit(true);
      ...
      {
        text: t('web.preview.edit.discard'),
        cls: 'btn-danger',
        action: function () {
          doExitEdit(true);
          activateTab(path);
...
function syncTreeActive(path) {
  document.querySelectorAll('#tree .active-node').forEach(function (el) {
    el.classList.remove('active-node', 'bg-blue-600', 'text-white');
  });
  if (!path || TempPreview.isTempPath(path)) return;
  var tree = document.getElementById('tree');
...
function saveTabsState() {
  try {
    var allTabs = tabStore.list().map(function (tab) {
      return tab.path;
    });
    var diskTabs = allTabs.filter(function (p) {
      return !TempPreview.isTempPath(p);
    });
...
function loadTabsState() {
  try {
    return JSON.parse(localStorage.getItem(tabsStorageKey()) || 'null');
  } catch (e) {
    return null;
  }
}
...
function restoreTabs(preferActivePath) {
  ...
  var active =
    preferActivePath || (st && st.active) || (tabs.length ? tabs[tabs.length - 1] : null);
  ...
  tabs.forEach(function (p) {
    if (!tabStore.has(p)) tabStore.open(p, basename(p));
  });
  renderTabBar();
...
function renderBreadcrumb(filePath) {
  ...
  if (!filePath) {
    bc.innerHTML =
      '<span id="breadcrumbPath" class="text-slate-400 text-sm">' +
      t('web.preview.toolbar.noFileSelected') +
      '</span>';
    return;
  }
  ...
  if (TempPreview.isTempPath(filePath)) {
    ...
    bc.innerHTML =
      ...
      t('web.preview.tempFile') +
      '</span><span class="text-slate-300 dark:text-slate-600 shrink-0">›</span><strong class="text-slate-700 dark:text-slate-200 truncate">' +
      esc(tName) +
      '</strong>';
    return;
  }
  var parts = filePath.split('/');
  var html =
    '<a href="javascript:void(0)" onclick="revealDirInTree(\'\')" class="hover:text-blue-500 dark:hover:text-blue-400 truncate shrink-0" title="' +
    t('web.preview.projectRoot') +
    '">' +
    esc(proj ? proj.name : t('web.preview.root')) +
    '</a>';
  var acc = '';
  parts.forEach(function (part, i) {
    ...
    if (i === parts.length - 1) {
      html +=
        '<strong class="text-slate-700 dark:text-slate-200 truncate">' + esc(part) + '</strong>';
    } else {
      acc += (acc ? '/' : '') + part;
      html +=
        '<a href="javascript:void(0)" onclick="revealDirInTree(\'' +
        escAttr(acc) +
        '\')" class="hover:text-blue-500 dark:hover:text-blue-400 truncate shrink-0">' +
        esc(part) +
        '</a>';
    }
  });
...
async function revealDirInTree(dirPath) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (i === parts.length - 1) {
      ...
      (function (el) {
        setTimeout(function () {
          el.classList.remove('breadcrumb-flash');
        }, 1500);
      })(row);
    }
  }
...
function highlightCode() {
  ...
  document
    .querySelectorAll('.doc-content pre code, .doc77-code-block pre code')
...
function loadMermaid(cb) {
  ...
  s.onload = function () {
    ...
    _mermaidCbs.splice(0).forEach(function (fn) {
      fn(true);
    });
  };
  s.onerror = function () {
    ...
    _mermaidCbs.splice(0).forEach(function (fn) {
      fn(false);
    }); // degrade: keep <pre class="mermaid"> source
  };
  document.head.appendChild(s);
}
...
function renderMermaid() {
  ...
  loadMermaid(function (ok) {
    ...
    try {
      window.mermaid.run({ querySelector: '#docPaneHost .mermaid:not([data-processed])' });
    } catch (e) {}
  });
}
...
function loadKaTeX(cb) {
  ...
  s.onload = function () {
    ...
    ar.onload = function () {
      _katexLoading = false;
      _katexCbs.splice(0).forEach(function (fn) {
        fn(true);
      });
    };
    ar.onerror = function () {
...
function getRecentFiles() {
  try {
    return JSON.parse(localStorage.getItem('doc77-recent') || '[]');
  } catch (e) {
    return [];
  }
}
function addRecentFile(fp) {
  ...
  rf = rf.filter(function (f) {
    return !(f.path === fp && f.pid == pid);
  });
...
function renderRecentFiles() {
  var rf = getRecentFiles().filter(function (f) {
    return f.pid == pid;
  });
  var el = document.getElementById('recentList');
  if (!rf.length) {
    el.innerHTML =
      '<div class="text-slate-600 text-xs px-1">' + t('web.preview.recent.empty') + '</div>';
    return;
  }
  el.innerHTML = rf
    .map(function (f) {
      return (
        '<div onclick="navigateToFile(\'' +
        escAttr(f.path) +
        '\')" class="flex items-center gap-1.5 px-2 py-1 rounded cursor-pointer hover:bg-slate-800 text-slate-300 truncate text-xs"><span>📄</span><span class="truncate">' +
        esc(f.path) +
        '</span></div>'
      );
    })
...
window.copyCode = function (btn) {
  ...
  if (!code) return;
  navigator.clipboard
    .writeText(code.innerText || code.textContent || '')
    .then(function () {
...
function onContentScroll() {
  ...
  if (btn) {
    btn.style.display = a.scrollTop > 300 ? 'flex' : 'none';
  }
}
...
function toggleTTS() {
  if (ttsActive) {
    ...
    document.getElementById('ttsRate').classList.add('hidden');
    return;
  }
  var text = (document.getElementById('docContent') || document.querySelector('.doc-content'))
    ?.textContent;
  if (!text) {
    toast(t('web.preview.openDocFirst'), 'error');
    return;
  }
  ttsActive = true;
  document.getElementById('ttsBtn').textContent = '⏹';
  ...
  u.lang = window.__doc77_lang || 'zh-CN';
  u.rate = rate;
  u.onend = function () {
    ttsActive = false;
    document.getElementById('ttsBtn').textContent = '🔊';
...
async function toggleTranslate() {
  ...
  if (!text || text.trim().length < 2) {
    toast(t('web.preview.docTooShort'), 'info');
    return;
  }

  var container = document.getElementById('contentArea');
  if (!container) {
    toast(t('web.preview.noContentArea'), 'error');
    return;
  }
  ...
  left.style.cssText =
    'flex:1;overflow-y:auto;padding:8px 12px;border-right:1px solid var(--border-light)';
  ...
  right.innerHTML =
    ...
    t('web.preview.translating') +
    '</div>';
  splitPane.appendChild(left);
  splitPane.appendChild(right);
  ...
  try {
    ...
    if (!r.ok) {
      ...
      right.innerHTML =
        ...
        (err.message || err.error || t('web.preview.translateFailed')) +
        '</div>';
      return;
    }
    var result = await r.json();
    var translatedHtml =
      '<div class="translated-content" style="white-space:pre-wrap">' +
...
(function initSelectionTranslate() {
  document.addEventListener('mouseup', function (e) {
    ...
    popup.style.cssText =
      'position:fixed;z-index:9999;background:var(--bg-card);border:1px solid var(--border);border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,0.15);padding:4px;display:flex;gap:4px;font-size:12px';
    popup.style.left = e.clientX + 10 + 'px';
    popup.style.top = e.clientY - 40 + 'px';
    var btn = document.createElement('button');
    btn.textContent = t('web.preview.translateSelected');
    btn.style.cssText =
      'padding:4px 10px;border:none;border-radius:6px;cursor:pointer;background:var(--accent);color:#fff;font-size:12px;white-space:nowrap';
    btn.onclick = async function () {
      clearTimeout(autoRemove);
      btn.textContent = t('web.preview.translating');
      btn.disabled = true;
      try {
        var r = await fetch('/api/translate', {
          method: 'POST',
        ...
        if (r.ok) {
          var result = await r.json();
          popup.innerHTML =
            '<div style="padding:8px 12px;max-width:360px;line-height:1.5;font-size:13px;color:var(--text-primary)">' +
            escapeHtml(result.translated_text) +
    ...
    var autoRemove = setTimeout(function () {
      if (document.getElementById('translatePopup')) popup.remove();
    }, 8000);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (sel)
    sel.addEventListener('change', function () {
      autoScrollSpeed = parseFloat(this.value) || 60;
    });
});
function toggleAutoScroll() {
  ...
  function step(time) {
    ...
    if (wholePx > 0) {
      scrollAccum -= wholePx;
      aCur.scrollTop += wholePx;
    }
    if (aCur.scrollTop >= aCur.scrollHeight - aCur.clientHeight) {
      toggleAutoScroll();
      return;
    }
...
async function doAISummary() {
  ...
  txt.textContent = t('web.preview.generating');
  try {
    var body = JSON.stringify({
      message: t('web.preview.summaryPrompt'),
    ...
    if (!res.ok) {
      txt.textContent = t('web.preview.aiUnavailable');
      return;
    }
    ...
    while (true) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (l.startsWith('data:')) {
          try {
            var dd = JSON.parse(l.slice(5).trim());
            if (dd.text) {
              summary += dd.text;
    ...
    txt.textContent = t('web.preview.summaryFailed') + ': ' + e.message;
  }
}
...
function toggleDocSearch() {
  ...
  if (!bar.classList.contains('hidden')) {
    ...
    document.getElementById('docSearchInput').select();
  } else {
...
function doDocSearch() {
  ...
  var doc = document.getElementById('docContent');
  if (!doc) {
    document.getElementById('searchCount').textContent = t('web.preview.zeroMatches');
    return;
  }
...
function highlightInNode(root, q) {
  ...
  textNodes.forEach(function (tn) {
    ...
    var frag = document.createDocumentFragment();
    var lastIdx = 0,
      originalText = tn.textContent;
    while (idx !== -1) {
      if (idx > lastIdx)
        frag.appendChild(document.createTextNode(originalText.substring(lastIdx, idx)));
...
document.addEventListener('keydown', function (e) {
  ...
  if (e.key === 'Escape' && !document.getElementById('docSearchBar').classList.contains('hidden')) {
    toggleDocSearch();
  }
});
...
async function doGlobalSearch() {
  ...
  rdiv.innerHTML =
    '<div class="text-xs text-slate-500 px-2 py-1">' + t('web.preview.searching') + '</div>';
  try {
    var res = await fetch('/api/search?q=' + encodeURIComponent(q) + '&project_id=' + pid);
    var d = await res.json();
    if (!d.matches.length) {
      rdiv.innerHTML =
        '<div class="text-xs text-slate-500 px-2 py-1">' + t('web.preview.noResults') + '</div>';
      return;
    }
    rdiv.innerHTML = d.matches
      .slice(0, 20)
      .map(function (m) {
        return (
          '<div onclick="navigateToFile(\'' +
          ...
          "', " +
          m.line +
          ')" class="flex flex-col px-2 py-1 rounded cursor-pointer hover:bg-slate-800 text-xs"><span class="text-blue-400 truncate">' +
          esc(m.file) +
          ':' +
          m.line +
          '</span><span class="text-slate-400 truncate">' +
          esc(m.content) +
          '</span></div>'
        );
      })
    ...
    rdiv.innerHTML =
      '<div class="text-xs text-red-500 px-2 py-1">' + t('web.preview.searchFailed') + '</div>';
  }
}
...
function getBookmarksSync() {
  ...
  try {
    return JSON.parse(localStorage.getItem('doc77-bookmarks') || '[]')
      .filter(function (b) {
        return b.pid == pid;
      })
      .map(function (b) {
        return { path: b.path, time: b.time };
      });
  } catch (e) {
    return [];
  }
...
function getBookmarks() {
  return fetch('/api/tree/' + pid + '/bookmarks')
    .then(function (r) {
      return r.json();
    })
    .then(function (d) {
...
function addBookmark(filePath) {
  fetch('/api/tree/' + pid + '/bookmark?path=' + encodeURIComponent(filePath), {
    ...
    .then(function () {
      if (_bookmarksCache)
        _bookmarksCache.unshift({ path: filePath, created_at: new Date().toISOString() });
      if (_bookmarksCache && _bookmarksCache.length > 50)
        _bookmarksCache = _bookmarksCache.slice(0, 50);
      renderBookmarks();
...
function removeBookmark(filePath) {
  fetch('/api/tree/' + pid + '/bookmark?path=' + encodeURIComponent(filePath), {
    ...
    .then(function () {
      if (_bookmarksCache)
        _bookmarksCache = _bookmarksCache.filter(function (b) {
...
function updateBookmarkPath(oldPath, newPath) {
  if (_bookmarksCache) {
    var entry = _bookmarksCache.find(function (b) {
      return b.path === oldPath;
    });
    if (entry) {
      entry.path = newPath;
    }
...
function renderBookmarks() {
  ...
  getBookmarks().then(function (bm) {
    ...
    if (!bm.length) {
      el.innerHTML =
        '<div class="text-slate-600 text-xs px-1">' + t('web.preview.bookmark.empty') + '</div>';
      return;
    }
    el.innerHTML = bm
      .map(function (b) {
        return (
          '<div class="flex items-center gap-1 px-2 py-1 rounded hover:bg-slate-800 group cursor-pointer">' +
          '<span class="truncate flex-1 text-slate-300" onclick="navigateToFile(\'' +
          escAttr(b.path) +
          '\')">⭐ ' +
          esc(b.path) +
          '</span>' +
          '<button class="hidden group-hover:block text-slate-500 hover:text-red-400 text-xs" onclick="event.stopPropagation();removeBookmark(\'' +
          escAttr(b.path) +
          '\')">✕</button></div>'
        );
      })
    ...
    fetch('/api/tree/' + pid + '/bookmarks/migrate', {
      ...
      .then(function (d) {
        if (d.imported > 0) {
          ...
          var remaining = all.filter(function (b) {
            return b.pid != pid;
          });
          try {
            localStorage.setItem('doc77-bookmarks', JSON.stringify(remaining));
          } catch (_) {}
...
function showCtxMenu(x, y, nodePath, nodeType) {
  ...
  if (isDir) {
    ...
    items.push({
      label: t('web.preview.ctxMenu.delete'),
      action: 'ctxDelete',
      danger: true,
      sep: true,
    });
    ...
    var isBookmarked = bm.some(function (b) {
      return b.path === nodePath;
    });
    items.push({
      label: isBookmarked
        ? t('web.preview.ctxMenu.bookmarkRemove')
        : t('web.preview.ctxMenu.bookmarkAdd'),
      action: 'ctxBookmark',
    });
    items.push({ label: t('web.preview.ctxMenu.rename'), action: 'ctxRename', sep: true });
    items.push({
  ...
  m.innerHTML = items
    .map(function (item) {
      var cls =
        'ctx-menu-item' +
        (item.danger ? ' ctx-menu-danger' : '') +
        (item.sep ? ' ctx-menu-sep' : '');
      return (
        '<button class="' +
        cls +
        '" data-action="' +
...
function ctxCreateFile(dirPath) {
  promptDialog({
    ...
    .catch(function (e) {
      toast(e.message || t('web.preview.toast.actionFailed'), 'error');
    });
}
...
function ctxRenameFile(oldPath) {
  ...
  promptDialog({
    ...
    .catch(function (e) {
      toast(e.message || t('web.preview.toast.renameFailed'), 'error');
    });
}
...
function ctxDeleteFile(targetPath, targetType) {
  ...
  confirmDialog(msg)
    ...
    .catch(function (e) {
      toast(e.message || t('web.preview.toast.deleteFailed'), 'error');
    });
}

function ctxToggleBookmark(filePath) {
  var bm = getBookmarksSync();
  var existing = bm.find(function (b) {
    return b.path === filePath;
  });
  if (existing) {
...
// Toolbar
function revealFile(action) {
  if (currentFile)
    fetch(
      '/api/reveal/' + pid + '?path=' + encodeURIComponent(currentFile) + '&action=' + action,
    ).catch(function () {});
}
function openAIChat() {
  if (!CAPABILITIES.ai) {
    toast(t('web.preview.aiNotInstalled'), 'info');
    return;
  }
  togglePanel('right');
  setActiveTab('chat');
  if (currentFile) {
    document.getElementById('ctxBanner').classList.remove('hidden');
    document.getElementById('ctxText').textContent = currentFile;
    sendQuickMsg(t('web.preview.chat.summarizePrompt', { file: currentFile }), currentFile);
  }
}
...
function enterEditMode() {
  ...
  docContent.innerHTML =
    ...
    '<div class="edit-pane-preview" id="editPreviewPane">' +
    previewHTML +
    '</div>' +
    '</div>' +
    '<div class="edit-statusbar" id="editStatusbar">' +
    '<span id="statusCursor">' +
    t('web.preview.edit.lineCol') +
    '</span>' +
    '<span class="status-sep"></span><span>' +
    lang.toUpperCase() +
    '</span>' +
    '<span class="status-sep"></span>' +
    '<span class="status-dirty" id="statusDirty" style="display:none">' +
    t('web.preview.edit.dirty') +
    '</span>' +
    '<span class="status-saved fade" id="statusSaved">' +
    t('web.preview.edit.saved') +
    '</span>' +
    '</div>';
  ...
  while (p && !p.classList.contains('max-w-4xl')) {
    p = p.parentElement;
  }
  if (p) {
    p.style.maxWidth = 'none';
    window._editMaxWidthParent = p;
  }
  ...
  var editBtnEl = document.getElementById('editBtn');
  if (editBtnEl) {
    editBtnEl.classList.add('editing-active');
    editBtnEl.title = t('web.preview.edit.exitEditMode');
  }
  editMode = true;
  editDirty = false;
  ...
  fetch('/api/raw/' + proj.id + '?path=' + encodeURIComponent(currentFile) + '&t=' + Date.now())
    ...
    .then(function (txt) {
      if (editMode) initEditorInstance(txt);
    })
    .catch(function () {
      if (editMode) initEditorInstance('');
    });
}
...
async function initEditorInstance(initialText) {
  ...
  pane.innerHTML =
    '<div class="flex items-center justify-center h-full"><span class="text-sm text-slate-400">' +
    t('web.preview.edit.loadingEditor') +
    '</span></div>';
  ...
  if (el) {
    el.addEventListener('input', function () {
      if (!editDirty) {
        editDirty = true;
        document.getElementById('statusDirty').style.display = '';
      }
      scheduleAutoSave();
...
function doSave(cb, skipPreview) {
  ...
  fetch('/api/content/' + proj.id + '?path=' + encodeURIComponent(currentFile), {
    ...
    .then(function (r) {
      if (r.status === 409) {
        r.json().then(function (d) {
          showEditConfirm(
            t('web.preview.edit.externalModified'),
            d.error || t('web.preview.edit.overwriteWarning'),
            [
              {
                text: t('web.preview.edit.overwriteSave'),
                cls: 'btn-danger',
                action: function () {
                  ...
                  fetch('/api/content/' + proj.id + '?path=' + encodeURIComponent(currentFile), {
                    ...
                    .then(function (d2) {
                      editModifiedTime = d2.modified;
                      markSaved();
                      if (!skipPreview) updateEditPreview(content);
                      if (cb) cb();
                    })
                    .catch(function (e) {
                      toast(t('web.preview.edit.saveFailed', { message: e.message }), 'error');
                    });
                },
              },
              { text: t('web.preview.edit.cancel'), cls: '' },
      ...
      return r.json().then(function (d) {
        if (!r.ok) throw new Error(d.error || t('web.preview.edit.saveFailed'));
        return d;
      });
    })
    .then(function (d) {
      if (!d) return;
      editModifiedTime = d.modified;
      markSaved();
      // Only refresh server-rendered preview on manual save (Ctrl+S), not auto-save
      if (!skipPreview) updateEditPreview(content);
      tabDataCache[currentFile] = {
        content: content,
        path: currentFile,
        size: d.size,
        modified: d.modified,
    ...
    .catch(function (e) {
      toast(t('web.preview.edit.saveFailed') + ': ' + e.message, 'error');
    });
}

function markSaved() {
  editDirty = false;
  var d = document.getElementById('statusDirty');
  if (d) d.style.display = 'none';
  var s = document.getElementById('statusSaved');
  if (s) {
    s.textContent = t('web.preview.edit.saved');
  ...
  try {
    ...
    if (ext === 'md' || ext === 'markdown' || ext === 'mdx') {
      ...
      if (window.marked) {
        ...
        pp.innerHTML =
          '<pre style="white-space:pre-wrap;font-size:14px;padding:16px 20px">' +
          escapeHtml(content) +
          '</pre>';
      }
    } else {
      pp.innerHTML =
        '<pre style="white-space:pre-wrap;font-size:14px;padding:16px 20px"><code>' +
        escapeHtml(content) +
        '</code></pre>';
...
function updateEditPreview(content) {
  ...
  fetch('/api/content/' + proj.id + '?path=' + encodeURIComponent(currentFile) + '&t=' + Date.now())
    ...
    .catch(function () {
      pp.innerHTML =
        '<pre style="white-space:pre-wrap;font-size:14px">' + escapeHtml(content) + '</pre>';
    });
}
...
function exitEditMode(skipConfirm) {
  ...
  if (editDirty && !skipConfirm) {
    showEditConfirm(t('web.preview.edit.unsavedTitle'), t('web.preview.edit.exitSavePrompt'), [
      {
        text: t('web.preview.edit.saveAndExit'),
        cls: 'btn-primary',
        action: function () {
          doSave(function () {
            doExitEdit();
...
function doExitEdit(skipRefresh) {
  ...
  if (window._editMaxWidthParent) {
    window._editMaxWidthParent.style.maxWidth = '';
    window._editMaxWidthParent = null;
  }
  ...
  if (window._editEditor) {
    try {
      window._editEditor.destroy();
    } catch (e) {}
    window._editEditor = null;
  }
  ...
  editMode = false;
  editDirty = false;
  editModifiedTime = null;
  clearTimeout(editAutoSaveTimer);
  editAutoSaveTimer = null;
  clearTimeout(window._editPreviewTimer);
  window._editPreviewTimer = null;
  var eb = document.getElementById('editBtn');
  if (eb) {
    eb.classList.remove('editing-active');
    eb.title = t('web.preview.edit.editFile');
  }
...
function initEditDivider() {
  ...
  var onMove = function (e) {
    ...
    var pct = sc.classList.contains('vertical')
      ? ((e.clientY - r.top) / r.height) * 100
      : ((e.clientX - r.left) / r.width) * 100;
    pct = Math.max(20, Math.min(80, pct));
    editSplitRatio = Math.round(pct);
    ep.style.flex = '0 0 ' + editSplitRatio + '%';
  };
  var onUp = function () {
    if (!dragging) return;
    dragging = false;
...
function cleanupEditDivider() {
  if (window._editDividerCleanup) {
    window._editDividerCleanup();
    window._editDividerCleanup = null;
  }
}

function showEditConfirm(title, message, buttons) {
  var ov = document.createElement('div');
  ov.className = 'edit-confirm-overlay';
  ov.innerHTML =
    '<div class="edit-confirm-dialog"><h3>' +
    escapeHtml(title) +
    '</h3><p>' +
    escapeHtml(message) +
    ...
    buttons
      .map(function (b, i) {
        return (
          '<button class="' + b.cls + '" data-idx="' + i + '">' + escapeHtml(b.text) + '</button>'
        );
      })
      .join('') +
    '</div></div>';
...
function buildOutline() {
  var doc = document.getElementById('docContent'),
    list = document.getElementById('outlineList');
  if (!doc) {
    list.innerHTML =
      '<div class="text-center py-12 text-slate-400 dark:text-slate-500 text-xs">' +
      t('web.preview.outline.openMarkdown') +
      '</div>';
    outlineBuilt = false;
    return;
  }
  ...
  headings.forEach(function (h, i) {
    h.id = 'heading-' + i;
    outlineHeadings.push({ id: h.id, text: h.textContent || '', level: parseInt(h.tagName[1]) });
  });
  if (!outlineHeadings.length) {
    list.innerHTML =
      '<div class="text-center py-12 text-slate-400 dark:text-slate-500 text-xs">' +
      t('web.preview.outline.noHeadings') +
      '</div>';
    outlineBuilt = true;
    return;
  }
  list.innerHTML = outlineHeadings
    .map(function (h) {
      return (
        '<div onclick="scrollToHeading(\'' +
        h.id +
        '\')" class="flex items-center gap-1.5 py-1.5 px-2 rounded-md cursor-pointer transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 text-sm" style="padding-left:' +
        ((h.level - 1) * 12 + 8) +
        'px">' +
        '<span class="w-1 h-4 rounded-full ' +
        (h.level === 1 ? 'bg-blue-500' : h.level === 2 ? 'bg-slate-400' : 'bg-slate-300') +
        ' shrink-0"></span>' +
...
async function sendMessage() {
  ...
  if (!msg) return;
  var ctxFile = pendingContextFile || currentFile;
  ...
  var aiMsg = appendChatMsg('ai', ''),
    aiBody = aiMsg.querySelector('.msg-body');
  try {
    var body = JSON.stringify({
      message: msg,
      project_id: parseInt(pid),
      session_id: chatSessionId || undefined,
      context_file: ctxFile || undefined,
    });
    ...
    if (!response.ok) {
      ...
      if (errData.error === 'AI_NOT_CONFIGURED')
        aiBody.innerHTML =
          ...
          t('web.preview.chat.configureAi') +
          '</span> <button onclick="toggleSettings();switchSettingsTab(\'ai\')" class="text-blue-600 underline text-xs">' +
          t('web.preview.chat.goToSettings') +
          '</button>';
      else
        aiBody.textContent =
          '❌ ' + (errData.message || errData.error || t('web.preview.chat.aiUnavailable'));
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (trimmed.startsWith('event:')) {
          ...
          try {
            ...
            handleSSE(currentEvent, data, aiBody);
          } catch (e) {}
        }
      }
    }
  } catch (e) {
    if (!aiBody.textContent) aiBody.textContent = t('web.preview.chat.networkFailed');
  }
  document.getElementById('sendBtn').disabled = false;
}
function handleSSE(event, data, aiBody) {
  switch (event) {
    ...
    case 'tool_call':
      ...
      var isWriteOp =
        ['move_file', 'create_folder', 'delete_file', 'batch_operations'].indexOf(data.name) >= 0;
      var ind = document.createElement('div');
      ind.dataset.tool = data.name;
      if (isWriteOp) {
        ind.dataset.write = '1';
        ...
        ind.innerHTML =
          '<span>📋 ' +
          esc(data.name) +
          t('web.preview.chat.pendingApproval') +
          '</span><a onclick="setActiveTab(\'queue\');loadTasks();" class="text-blue-600 dark:text-blue-400 underline cursor-pointer">' +
          t('web.preview.chat.viewQueue') +
          '</a>';
      } else {
        ind.className = 'text-xs text-blue-600 dark:text-blue-400 mt-1 flex items-center gap-1';
        ind.innerHTML = '<span>🔍 ' + esc(data.name) + '...</span>';
      }
      aiBody.appendChild(ind);
      break;
    case 'done':
      aiBody.querySelectorAll('[data-tool]:not([data-write])').forEach(function (el) {
        el.innerHTML =
          '<span>' +
          t('web.preview.chat.toolCompleted', { tool: esc(el.dataset.tool) }) +
          '</span>';
        el.className = 'text-xs text-green-600 dark:text-green-400 mt-1';
      });
      break;
    case 'error':
      var ed = document.createElement('div');
      ed.className = 'text-xs text-red-500 mt-1';
      ed.textContent = '❌ ' + (data.message || t('web.preview.chat.unknownError'));
      aiBody.appendChild(ed);
      break;
  }
}
function appendChatMsg(role, content) {
  ...
  d.innerHTML =
    ...
    (role === 'user' ? 'bg-blue-100 text-blue-600' : 'bg-slate-800 text-white') +
    '">' +
    (role === 'user' ? '👤' : '🤖') +
    '</div>' +
    '<div class="p-3 rounded-lg text-sm max-w-[85%] whitespace-pre-wrap leading-relaxed msg-body ' +
    (role === 'user'
...
function sendQuickMsg(m, contextFile) {
  pendingContextFile = contextFile || null;
...
function doSmartClassify() {
  ...
  if (!CAPABILITIES.mcp) {
    toast(t('web.preview.toolbar.classifyNeedsMCP'), 'info');
    return;
  }
...
async function loadTasks() {
  ...
  try {
    var r = await fetch('/api/queue/status?project_id=' + pid);
    ...
    if (!tasks.length) {
      list.innerHTML =
        '<div class="text-center py-16 text-slate-400 text-sm flex flex-col items-center gap-2">✅<br>' +
        t('web.preview.queue.empty') +
        '</div>';
      return;
    }
    var tl = {
      write_file: t('web.preview.queue.type.writeFile'),
      create_folder: t('web.preview.queue.type.createFolder'),
      move_file: t('web.preview.queue.type.moveFile'),
      delete_file: t('web.preview.queue.type.deleteFile'),
      batch_operations: t('web.preview.queue.type.batchOperations'),
    };
    var tc = {
      write_file: 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300',
      create_folder: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300',
      move_file: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',
      delete_file: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',
      batch_operations: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300',
    };
    list.innerHTML = tasks
      .map(function (task) {
        var label = tl[task.operation_type] || task.operation_type,
          tcl = tc[task.operation_type] || 'bg-slate-100 text-slate-700';
        var isP = task.status === 'pending',
          isX = task.status === 'executing',
          isE = task.status === 'executed',
          isR = task.status === 'rejected';
        var borderCls = isE
        ...
        var stHtml = isP
          ? '<div class="flex gap-1"><button onclick="approveTask(\'' +
            task.id +
            '\')" class="p-1 text-green-600 hover:bg-green-100 rounded">✅</button><button onclick="rejectTask(\'' +
            task.id +
            '\')" class="p-1 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded">✕</button></div>'
          : isX
            ? '<span class="text-xs font-bold text-blue-600">' +
              t('web.preview.task.executing') +
              '</span>'
            : isE
              ? '<span class="text-xs font-bold text-emerald-600">' +
                t('web.preview.task.executed') +
                '</span>'
              : '<span class="text-xs font-bold text-slate-400">' +
                t('web.preview.task.rejected') +
                '</span>';
        var d =
          typeof task.operation_data === 'string'
            ? JSON.parse(task.operation_data)
            : task.operation_data;
        var pi = '';
        if (d.file_path) pi += '<div>📄 ' + esc(d.file_path) + '</div>';
        if (d.folder_path) pi += '<div>📁 ' + esc(d.folder_path) + '</div>';
        if (d.source) pi += '<div class="opacity-70 line-through">' + esc(d.source) + '</div>';
        if (d.target) pi += '<div class="text-blue-700">→ ' + esc(d.target) + '</div>';
        return (
          '<div class="bg-white dark:bg-slate-800 border rounded-lg p-3 shadow-sm relative overflow-hidden ' +
          borderCls +
          ' ' +
          (isX ? 'dark:border-blue-700' : 'dark:border-slate-700') +
          '">' +
          (isX
            ? '<div class="absolute top-0 left-0 h-1 bg-blue-500 w-full" style="animation:pulse 1s infinite"></div>'
            : '') +
          '<div class="flex items-start justify-between mb-2"><div class="flex items-center gap-1.5 text-xs font-semibold"><span class="' +
          tcl +
          ' px-1.5 py-0.5 rounded text-[10px] font-bold">' +
          label +
          '</span></div>' +
          stHtml +
          '</div><div class="text-[11px] text-slate-600 dark:text-slate-400 space-y-1 font-mono bg-slate-50 dark:bg-slate-900 p-2 rounded border">' +
          (pi || JSON.stringify(d)) +
          '</div></div>'
        );
      })
...
async function approveAll() {
  ...
  for (var i = 0; i < tasks.length; i++) {
    if (tasks[i].status === 'pending') await approveTask(tasks[i].id);
  }
}
async function rejectAll() {
  ...
  var tasks = await r.json();
  for (var i = 0; i < tasks.length; i++) {
    if (tasks[i].status === 'pending') await rejectTask(tasks[i].id);
  }
}
...
document.addEventListener('fullscreenchange', function () {
  ...
  if (document.fullscreenElement === el) {
    ...
    el.style.inset = '0';
    el.style.zIndex = '100';
    el.style.background = '#525659';
    el.style.borderRadius = '0';
    el.style.minHeight = '100vh';
    el.style.padding = '16px';
  } else if (!document.fullscreenElement && _pdfFsOriginals) {
    for (var k in _pdfFsOriginals) {
      el.style[k] = _pdfFsOriginals[k];
    }
...
document.addEventListener('fullscreenchange', function () {
  ...
  if (document.fullscreenElement === el) {
    ...
    for (var k in _htmlFsOriginals) {
      el.style[k] = _htmlFsOriginals[k];
    }
      ...
      .then(function (buf) {
        mammoth
          ...
          .then(function (result) {
            var html =
              '<div class="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-8 sm:p-12 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700"><div class="doc-content text-slate-700 dark:text-slate-300" id="docContent">' +
              result.value +
              '</div></div>';
          ...
          .catch(function () {
            area.innerHTML =
              ...
              t('web.preview.office.docxFailed') +
              '</div>';
          });
      });
  });
...
function renderXlsx(d, area, path) {
  if (typeof XLSX === 'undefined') {
    ...
    s.onload = function () {
      renderXlsx(d, area, path);
    };
    document.head.appendChild(s);
    area.innerHTML =
      '<div class="h-full flex items-center justify-center"><div class="text-sm text-slate-500">' +
      t('web.preview.office.loadingXlsx') +
      '</div></div>';
    return;
  }
  fetch(d.rawUrl)
    ...
    .then(function (buf) {
      ...
      wb.SheetNames.forEach(function (name, i) {
        tabsHtml +=
          '<button onclick="switchXlsxSheet(\'' +
          escAttr(name) +
          '\')" class="xlsx-tab px-3 py-1 text-xs rounded-t-md border ' +
          (i === 0
            ? 'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 font-semibold'
      ...
      area.innerHTML =
        '<div class="p-2 sm:p-4"><div class="max-w-full mx-auto bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">' +
        tabsHtml +
        sheetsHtml +
        '</div></div>';
    })
    .catch(function () {
      area.innerHTML =
        '<div class="h-full flex items-center justify-center text-slate-400">' +
        t('web.preview.office.xlsxFailed') +
        '</div>';
    });
}

function switchXlsxSheet(name) {
  document.querySelectorAll('.xlsx-sheet').forEach(function (el) {
  ...
  document.querySelectorAll('.xlsx-tab').forEach(function (el) {
    ...
    el.className =
      'xlsx-tab px-3 py-1 text-xs rounded-t-md border ' +
      (active
        ? 'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 font-semibold'
        : 'bg-slate-100 dark:bg-slate-800 text-slate-500');
  });
...
function openImageLightbox(url, filePath) {
  getSiblingImages(filePath).then(function (siblings) {
    ...
    overlay.innerHTML =
      ...
      '<img src="' +
      url +
      '" class="max-w-[95vw] max-h-[85vh] object-contain transition-transform duration-200" id="lightboxImg">' +
      ...
      '<button onclick="lightboxNav(-1)" class="w-8 h-8 rounded-full hover:bg-white/20 ' +
      (siblings.length > 1 ? '' : 'opacity-30') +
      '">◀</button>' +
    ...
    window._lightboxIdx = siblings.findIndex(function (s) {
...
function lightboxZoom(delta) {
  ...
  document.getElementById('lightboxZoomPct').textContent =
    Math.round(window._lightboxZoom * 100) + '%';
}
...
function lightboxNav(dir) {
  ...
  document.getElementById('lightboxImg').src =
    '/api/raw/' + pid + '?path=' + encodeURIComponent(next.path);
...
function updateLightboxCount() {
  ...
  if (el)
    el.textContent = window._lightboxIdx + 1 + ' / ' + (window._lightboxSiblings || []).length;
}
...
function getSiblingImages(filePath) {
  ...
  return fetch('/api/tree/' + pid + '?path=' + encodeURIComponent(parentDir || ''))
    ...
    .then(function (d) {
      var images = d.entries.filter(function (e) {
        return (
          e.type === 'file' &&
          imgExts.some(function (ext) {
            return e.name.toLowerCase().endsWith(ext);
          })
        );
      });
      ...
      return images.map(function (e) {
        return { name: e.name, path: (parentDir ? parentDir + '/' : '') + e.name };
      });
    })
    .catch(function () {
      return [];
...
function runCode() {
  ...
  if (!code.trim()) {
    toast(t('web.preview.codeRun.noCode'), 'error');
    return;
  }

  var area =
    document.querySelector('#docPaneHost .doc-pane') || document.getElementById('contentArea');
  ...
  outputDiv.className =
    'max-w-4xl mx-auto mt-4 bg-slate-900 dark:bg-black border border-slate-700 rounded-xl overflow-hidden';
  outputDiv.innerHTML =
    '<div class="flex items-center justify-between px-4 py-2 bg-slate-800 dark:bg-slate-950 border-b border-slate-700 text-xs text-slate-300">' +
    '<span>' +
    t('web.preview.codeRun.output') +
    '</span><span class="text-slate-500" id="codeOutputLang">' +
    lang +
    '</span>' +
    '<button onclick="document.getElementById(\'codeOutput\').remove()" class="text-slate-400 hover:text-white">✕</button></div>' +
    '<div class="p-4 font-mono text-sm text-emerald-400 whitespace-pre-wrap max-h-80 overflow-y-auto" id="codeOutputText">' +
    t('web.preview.codeRun.running') +
    '</div>';
...
function runJS(code) {
  ...
  try {
    iframe.contentWindow.console = {
      log: function () {
        for (var i = 0; i < arguments.length; i++) output.push(String(arguments[i]));
      },
    };
...
function initPyodide(cb) {
  ...
  var pyodideUrl = window.__VENDOR_READY
    ? '/vendor/'
...
async function exportHTML() {
  ...
  btn.disabled = true;
  btn.innerHTML =
    '⏳ <span class="hidden sm:inline">' + t('web.preview.toolbar.exportPacking') + '</span>';
  ...
  try {
    ...
    btn.disabled = false;
    btn.innerHTML =
      '📥 <span class="hidden sm:inline">' + t('web.preview.toolbar.export') + '</span>';
  }
}
...
async function shareDocument() {
  ...
  btn.disabled = true;
  btn.innerHTML =
    '⏳ <span class="hidden sm:inline">' + t('web.preview.share.creating') + '</span>';
  ...
  try {
    ...
    btn.disabled = false;
    btn.innerHTML =
      '🔗 <span class="hidden sm:inline">' + t('web.preview.toolbar.share') + '</span>';
  }
}
...
function showShareCard(url, expiresAt, title) {
  ...
  card.innerHTML =
    ...
    url.replace(/'/g, "\\'") +
    '\')" style="flex:1;padding:8px 0;font-size:13px;font-weight:500;background:var(--accent,#6366f1);color:#fff;border:none;border-radius:8px;cursor:pointer">' +
    t('web.preview.share.copyLink') +
    '</button>' +
    '<button onclick="showQRCode(this,\'' +
    token +
    '\')" style="flex:1;padding:8px 0;font-size:13px;font-weight:500;background:var(--bg-card,#fff);color:var(--text-primary,#1e293b);border:1px solid var(--border-light,#e2e8f0);border-radius:8px;cursor:pointer">' +
    t('web.preview.share.qrcode') +
    '</button>' +
    '</div>' +
    ...
    '<div style="margin-top:14px;font-size:10px;color:#ef4444">' +
    t('web.preview.share.wifiWarning') +
    '</div>' +
    '</div>';
