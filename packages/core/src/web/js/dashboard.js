...
window.renderGrid = function (items) {
  ...
  if (!items.length) {
    grid.innerHTML = filterText
      ? '<div class="empty-state"><div class="empty-text">没有匹配 "' +
        esc(filterText) +
        '" 的项目</div></div>'
...
function renderCompactCard(p, inFavorites) {
  ...
  var dateLabel =
    sortBy === 'last_opened' && p.last_opened
      ? '最近: ' +
        new Date(p.last_opened).toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })
      : '创建: ' +
  ...
  return (
    '<div class="card card-compact animate-in" data-id="' +
    p.id +
    '" onclick="openProject(' +
    p.id +
    ')">' +
    ...
    obsidianBadge +
    '</div>' +
    '<div class="card-path" title="' +
    escAttr(p.path) +
    '">' +
    esc(shortPath(p.path)) +
    '</div>' +
    '<div class="card-date">' +
    dateLabel +
    '</div>' +
    ...
    '<button class="btn-icon" onclick="event.stopPropagation();startEdit(' +
    p.id +
    ')" title="编辑">✏️</button>' +
    '<button class="btn-icon" onclick="event.stopPropagation();doDelete(' +
    p.id +
    ')" title="删除">🗑</button>' +
    '</div>' +
    ...
    '" value="' +
    escAttr(p.name) +
    '" placeholder="项目名称" class="input" style="width:100%;margin-bottom:8px">' +
    '<div class="form-row" style="margin-bottom:8px">' +
    '<input id="editPath-' +
    p.id +
    '" value="' +
    escAttr(p.path) +
    '" placeholder="项目路径" class="input">' +
    '<button onclick="openDirDialog(' +
    p.id +
    ')" class="btn">📂</button>' +
    '</div>' +
    '<label style="display:flex;align-items:center;gap:6px;font-size:12px;margin-bottom:8px">' +
    '<input type="checkbox" id="editObsidian-' +
    p.id +
    '" ' +
    (p.obsidian_mode ? 'checked' : '') +
    '>' +
    ' Obsidian vault</label>' +
    '<div style="display:flex;gap:8px">' +
    '<button onclick="doUpdate(' +
    p.id +
    ')" class="btn btn-primary" style="font-size:12px">💾 保存</button>' +
    '<button onclick="cancelEdit(' +
    p.id +
    ')" class="btn" style="font-size:12px">✕ 取消</button>' +
    '</div>' +
    '<div id="editError-' +
    p.id +
    '" style="color:var(--danger);font-size:12px;display:none;margin-top:6px"></div>' +
    '</div>' +
    '</div>'
  );
}
...
window.openProject = function (id) {
  ...
  if (navigator.sendBeacon) {
    ...
    fetch('/api/projects/' + id + '/touch', { method: 'POST', keepalive: true }).catch(
      function () {},
    );
  }
...
window.renderRecent = async function () {
  ...
  try {
    ...
    if (hrc)
      hrc.textContent = recentFiles.filter(function (f) {
        return f.viewedAt;
      }).length;
      ...
      recentProjects.forEach(function (p) {
        parts.push(
          '<a href="/preview.html?id=' +
            p.id +
            '" class="recent-pill" title="' +
            escAttr(p.name) +
            '" onclick="fetch(\'/api/projects/' +
            p.id +
            "/touch',{method:'POST'})\">📂 " +
            esc(p.name) +
            '</a>',
    ...
    if (recentFiles.length) {
      ...
      recentFiles.forEach(function (f) {
        ...
        parts.push(
          '<a href="' +
            href +
            '" class="recent-file-link" title="' +
            escAttr(f.projectName + ' / ' + f.fileName) +
            '">📄 ' +
            label +
            '</a>',
        );
...
window.doRegister = async function () {
  ...
  try {
    ...
    var r = await fetch('/api/projects', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: name, path: pth, obsidian_mode: isObsidian }),
    });
    if (!r.ok) {
      var d = await r.json();
...
window.navigateFsBrowser = function (forEditId, dirPath) {
  ...
  overlay.innerHTML =
    '<div class="confirm-box" style="max-width:600px;max-height:80vh;display:flex;flex-direction:column" id="fsBrowserBox"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-shrink:0"><span style="font-weight:600;font-size:13px">📂 服务端文件浏览</span><button onclick="document.getElementById(\'fsBrowserOverlay\').remove()" class="btn-icon" style="font-size:18px">✕</button></div><div id="fsBrowserContent" style="font-size:11px;color:var(--text-muted)">加载中...</div></div>';
  ...
  fetch(url)
    ...
    .then(function (d) {
      if (d.error) {
        document.getElementById('fsBrowserContent').innerHTML =
          '<p style="color:var(--danger)">❌ ' + esc(d.error) + '</p>';
        return;
      }
...
function renderFsBrowser(forEditId, data) {
  ...
  // Breadcrumb + back
  html +=
    '<div style="display:flex;align-items:center;gap:4px;margin-bottom:8px;flex-wrap:wrap;font-size:12px">';
  if (data.parent) {
    html +=
      '<button onclick="navigateFsBrowser(' +
      forEditId +
      ",'" +
      escAttr(data.parent) +
      '\')" style="padding:2px 8px;background:var(--bg-hover);border:1px solid var(--border-light);border-radius:4px;cursor:pointer;font-size:11px;color:var(--text-primary)">⬆ 上级</button>';
  }
  // Root shortcuts
  if (data.roots && data.roots.length > 0) {
    for (var ri = 0; ri < data.roots.length; ri++) {
      ...
      html +=
        ...
        ",'" +
        escAttr(root) +
        '\')" style="padding:2px 8px;border:1px solid var(--accent);border-radius:4px;cursor:pointer;font-size:10px;color:var(--accent);background:var(--accent-light-bg)">' +
        esc(root) +
        '</button>';
    }
  }
  ...
  // Current path
  html +=
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:10px;color:var(--text-muted)">当前:</span><span style="font-size:12px;font-family:monospace;color:var(--text-secondary);background:var(--bg-hover);padding:2px 8px;border-radius:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' +
    esc(data.path) +
    '</span></div>';

  // "Select this directory" button
  html +=
    '<button onclick="fillPath(' +
    forEditId +
    ",'" +
    escAttr(data.path) +
    "');document.getElementById('fsBrowserOverlay').remove();toast('路径已选择: " +
    escAttr(data.path) +
    '\',\'success\')" style="width:100%;padding:8px 0;margin-bottom:8px;background:#059669;color:#fff;font-size:13px;border:none;border-radius:6px;cursor:pointer;font-weight:500">✅ 选择此目录</button>';

  // Directory listing
  if (data.entries.length === 0) {
    html +=
      '<p style="font-size:12px;color:var(--text-muted);padding:16px 0;text-align:center">此目录为空</p>';
  } else {
    html +=
      '<div style="overflow-y:auto;flex:1;max-height:300px"><div style="display:flex;flex-direction:column;gap:2px">';
    for (var i = 0; i < data.entries.length; i++) {
      ...
      if (e.type === 'directory') {
        html +=
          ...
          forEditId +
          ",'" +
          escAttr(data.path + '/' + e.name) +
          '\')" style="width:100%;text-align:left;padding:6px 12px;font-size:13px;border:none;border-radius:6px;cursor:pointer;display:flex;align-items:center;gap:8px;color:var(--text-primary);background:transparent" onmouseover="this.style.background=\'var(--accent-light-bg)\'" onmouseout="this.style.background=\'transparent\'"><span>📁</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' +
          esc(e.name) +
          '</span></button>';
      }
    }
