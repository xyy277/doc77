...
export function registerPrompts(server: McpServer): void {
  server.prompt('organize-project', '帮我整理项目目录结构', async () => ({
    messages: [
      {
        role: 'user' as const,
        content: {
...
5. 每个操作建议需说明意图和理由`,
  ...
  server.prompt('search-and-classify', '搜索并归类文档', async () => ({
    messages: [
      {
        role: 'user' as const,
        content: {
...
4. 生成具体的 move_file / batch_operations 操作建议`,
  ...
  server.prompt('summarize-project', '生成项目摘要', async () => ({
    messages: [
      {
        role: 'user' as const,
        content: {
...
- 建议的改进方向（如：哪些文档可以合并、哪些目录需要整理）`,
  ...
  server.prompt('find-duplicates', '查找重复或相似文件', async () => ({
    messages: [
      {
        role: 'user' as const,
        content: {
