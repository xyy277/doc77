...
| 24 | v1.1.0 Release Electron 打包失败（linux AppImage） | `⨯ appimage.AppImageConfiguration.FileAssociations: ... FileAssociation.Ext: ReadString: expects " or n, but found [ ... "fileAssociations":[{"ext":["md","mdx","markdown"]}...` | electron-builder v25 的 AppImage 目标不支持 `fileAssociations[].ext` 数组（仅接受单个字符串，已知 issue electron-userland/electron-builder#4612）；fileAssociations 配置在 `package.json` `build` 顶层对所有平台生效，AppImage 解析 ext 数组时崩溃（NSIS/macOS 支持数组） | fileAssociations 从 `package.json` `build` 顶层移到 `electron-builder.yml` 的 `win`/`mac` 平台块（官方文档：fileAssociations 仅支持 macOS/NSIS/MSI）；Linux 不配置（官方用 `mimeType`，且 AppImage mime 打包位置有已知问题 #6664） |
| 25 | v1.1.0 Release Electron 二连败（snap 意外构建 + AppImage 工具崩溃） | `• building target=snap ...`（未配置 snap 却构建）+ `reason=linux.category is not set` + `⨯ expected argument for flag '--executable'` + `app-builder process failed ERR_ELECTRON_BUILDER_CANNOT_EXECUTE` | **electron-builder 的 loadConfig：package.json 存在 `build` 字段时只使用它，electron-builder.yml 完全被忽略（二选一，不合并）**。yml 的 `linux.target=AppImage` / `category` / `directories.output=release` / `extraResources` 从未生效 → Linux 用默认 target（snap + AppImage）、默认输出目录 `dist/`，与 CI 上传 glob `release/*` 不匹配；fileAssociations 修复也只到 yml 里，未加载时同样无效 | 删除 `packages/electron/package.json` 的 `build` 字段，让 `electron-builder.yml` 完全接管（yml 已含 appId/productName/files/win/mac/linux/extraResources/publish 全部配置）。防回归：修改后可用 `loadConfig` 验证 `configFile` 指向 electron-builder.yml；CI 日志若出现未配置的 target 即 yml 未生效 |
