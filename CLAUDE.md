...
**严禁默认发布 npm 包。** 发布到 npm registry 必须满足以下条件：

1. **用户明确授权** — 只有用户明确说出"发布"、"publish"、"推到 npm"等关键词时才可执行 `npm publish`
2. **禁止自动发布** — 不得在修复 bug、添加功能后主动建议或执行发布，必须等用户指令
3. **禁止随 commit 发布** — git commit / push 流程与 npm publish 流程完全独立，不得联动
4. **发布前验证** — 发布前必须完成：
   - 所有测试通过（`pnpm test`）
   - 本地 `npm pack` 验证包内容正确
   - `npm install -g ./xxx.tgz` 本地安装测试通过
