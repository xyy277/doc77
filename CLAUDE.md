...
- **英文（默认/权威版本）**：`README.md`、`CHANGELOG.md`、`package.json#description`
- **中文翻译**：`README.zh-CN.md`，通过顶部语言切换 badge 关联
- **内部设计文档**（`docs/design/`、`docs/planning/`）：保持中文（面向中文开发团队）
...
|---|---|---|
| 1 | 用户在当前回合明确说了"发布" | ☐ |
| 2 | `pnpm test` 全部通过（当前 736 用例，2026-08-14 基线） | ☐ |
| 3 | `pnpm build` 全部成功 | ☐ |
...
- **构建**：`pnpm build`
- **启动开发服务**：`pnpm dev:start`（构建 + 启动，端口 27777，绑定 0.0.0.0 用于测试）
