...
| 模块 | 内容 |
|---|---|
| **多语言 UI** | 简体中文 / English 内置，自动检测浏览器与系统语言，可放置 `~/.doc77/locales/<lang>.json` 语言包扩展任意语言 |
| **导出与分享** | 一键导出自包含 HTML（内联样式与图片、深浅主题）、局域网只读分享链接（有效期 + 二维码 + 一键撤销） |
| **离线翻译** | Opus-MT ONNX 模型完全本地运行（en↔zh），划词即译、文档翻译，内容不出本机 |
| **移动伴侣** | Dashboard 扫码直达手机版，mDNS 局域网发现，移动端自适应 UI |
| **项目导入** | Obsidian vault（`[[wikilink]]` 解析）、Git 项目批量扫描注册、VS Code workspace 导入、技术栈标签识别 |
| **多 tab 预览** | 多文档标签页（LRU 渲染缓存）、临时文件拖放预览、轻量编辑与冲突检测 |
| **密码安全** | 信封加密（DEK）、10 个一次性恢复码、忘记密码流程、scrypt 密钥派生（N=131072） |
| **Electron 桌面版** | Windows / macOS / Linux 一键安装，原生文件对话框，系统托盘，vendor 资源内置 |
| **模块插件化** | AI / MCP / 翻译可选安装（`doc77 i ai` / `doc77 i translate`），默认轻量，按需扩展 |
| **安全与权限** | 黑名单路径拦截 + 密码保护 + 恢复码重置、暴力破解防护、审计日志、写操作审批队列 |
...
| **导出 / 分享** | 自包含 HTML 导出（保留主题底色与样式），局域网只读分享（token 有效期、二维码、可撤销） |
| **离线翻译** | Opus-MT ONNX 完全离线运行，自动语言检测，划词翻译 + 长文档分段翻译 |
| **项目导入** | Obsidian vault、Git 目录批量扫描、VS Code workspace 一键导入 |
| **多语言** | 界面 zh/en 双语，全局语言 + 浏览器级覆盖，外部语言包目录扩展 |
