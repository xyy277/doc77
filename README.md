...
| 模块 | 内容 |
|---|---|
| **轻量编辑** | 分屏编辑 + CodeMirror 6 + 实时预览 + 自动保存，无需离开 Doc77 即可快速修改文档 |
...
| **AI 写链路** | AI 可通过审批队列提议文件操作（创建/移动/删除/批量），你审批后执行 |
| **AI 会话持久化** | 聊天记录保存到 SQLite，重启服务后自动恢复，每会话限流保护 |
| **AI 智能归类** | 实时 tool_call 流式输出，智能归类建议、文档总结分析 |
| **AI Provider 设置 UI** | Web 界面直连配置 AI Token / Base URL / 模型，支持连接测试 |
...
| **UI 增强** | Glow ripple 登录过渡动画 + 全局加载 spinner + 遗留迁移进度条 |
| **DB 自动持久化** | 内存数据库自动持久化到磁盘，崩溃时数据不丢失 |
